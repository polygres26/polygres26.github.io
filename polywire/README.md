...
whichever Postgres `POLYWIRE_HOST` points at, auto-created on first boot. See `ConfigStore`'s and
`FirewallRuleStore`'s class javadoc for the full schema and examples.

## Primary/standby failover

Set `POLYWIRE_STANDBY_HOST`/`POLYWIRE_STANDBY_PORT` (a real physical-replica pair of the primary
...
Embedded Apache Ignite (used by `CacheStage`, only actually started when `POLYWIRE_CACHE_TABLES`
