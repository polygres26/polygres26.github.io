...
the same way the old two-image setup did, and don't set `SAYONORA_DMS_WEB_DIR` on the
...
lives in an embedded HSQLDB file store at `SAYONORA_DATA_DIR` (default `/data` in this image,
`~/.sayonora` outside a container — see `ConnectionStore`/`LlmSettingsStore`/`ReportStore`'s
